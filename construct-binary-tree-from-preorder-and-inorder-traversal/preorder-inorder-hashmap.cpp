...
//   - Elements left  of that position → left subtree  (inorder range [start,
//   pos-1])
//   - Elements right of that position → right subtree (inorder range [pos+1,
//   end])
//   - idx advances in preorder order (left subtree fully consumed before right)
//
// Time:  O(n) — each node visited once; map lookup O(1)
// Space: O(n) — hash map stores all n values; recursion stack O(h), h = tree
// height
...
public:
  ...
  TreeNode *buildBT(vector<int> &preorder, int start, int end) {
    if (start > end)
      return NULL; // empty range → no node
